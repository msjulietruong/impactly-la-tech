...
export async function getBetterAlternatives(req, res) {
  try {
    ...
    const product = await foodCollection.findOne({
    ...
    if (!product.embedding) {
      return res.status(400).json({
        error:
          "Product has no embedding. Run POST /api/embeddings/generate first",
      });
    }
...
export async function debugGradeStats(req, res) {
  try {
    ...
    const gradeStats = await foodCollection
      .aggregate([
        {
          $group: {
    ...
    const totalProducts = await foodCollection.countDocuments();
    const withEmbeddings = await foodCollection.countDocuments({
